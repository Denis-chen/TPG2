...
//	Wdm2Test example
/////////////////////////////////////////////////////////////////////////////
//	Wdm2Test.cpp:	Win32 console application to exercise Wdm2 devices
/////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char* argv[])
{
	...
	// Open device
	printf("\nTest %d\n",TestNo++);
	HANDLE hWdm2 = GetDeviceViaInterface((LPGUID)&WDM2_GUID,0);
	if( hWdm2==NULL)
	{
		printf("XXX  Could not find open Wdm2 device\n");
		return 1;
	}
	printf("     Opened OK\n");
	...
	DWORD TxdBytes;
	ULONG Rvalue = 0;
	if( !ReadFile( hWdm2, &Rvalue, 4, &TxdBytes, NULL))
	...
	if( !WriteFile( hWdm2, &Wvalue, 4, &TxdBytes, NULL))
	...
	// Set file pointer
	printf("\nTest %d\n",TestNo++);
	DWORD dwNewPtr = SetFilePointer( hWdm2, 3, NULL, FILE_BEGIN);
	...
	printf("\nTest %d\n",TestNo++);
	Rvalue = 0;
	if( !ReadFile( hWdm2, &Rvalue, 1, &TxdBytes, NULL))
	...
	ULONG BufferSize;
	DWORD BytesReturned;
	if( !DeviceIoControl( hWdm2, IOCTL_WDM1_GET_BUFFER_SIZE,
							NULL, 0,					// Input
	...
	if( !DeviceIoControl( hWdm2, IOCTL_WDM1_GET_BUFFER,
							NULL, 0,			// Input
	...
	printf("\nTest %d\n",TestNo++);
	if( !DeviceIoControl( hWdm2, IOCTL_WDM1_ZERO_BUFFER,
							NULL, 0,	// Input
							NULL, 0,	// Output
							&BytesReturned, NULL))
	...
	// Remove buffer
	printf("\nTest %d\n",TestNo++);
	if( !DeviceIoControl( hWdm2, IOCTL_WDM1_REMOVE_BUFFER,
							NULL, 0,	// Input
							NULL, 0,	// Output
							&BytesReturned, NULL))
	...
	// Unrecognised IOCTL
	printf("\nTest %d\n",TestNo++);
	if( !DeviceIoControl( hWdm2, IOCTL_WDM1_UNRECOGNISED,
							NULL, 0,	// Input
							NULL, 0,	// Output
							&BytesReturned, NULL))
	...
	printf("\nTest %d\n",TestNo++);
	dwNewPtr = SetFilePointer( hWdm2, 0, NULL, FILE_BEGIN);
	if( dwNewPtr==0xFFFFFFFF)
		printf("XXX  SetFilePointer failed %d\n",GetLastError());
	else
		printf("     SetFilePointer worked\n");
